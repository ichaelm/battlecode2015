package minerbot_maxoremindist_picky_savedmarkedminer3;
...
public class RobotPlayer {
	...
	static LinkedList<MapLocation> prevPath = new LinkedList<MapLocation>();
	...
	*/
	...
	public static void run(RobotController myrc) {
		...
		brandNew = true;
		for (int i = 0; i < 2; i++) {
            prevPath.addLast(new MapLocation(-1, -1));
        }
    	prevPath.addLast(rc.getLocation());
	...
	private static void moveTo(MapLocation targetLocation) throws GameActionException {
    	...
    	HashSet<MapLocation> prevPathSet = new HashSet<MapLocation>(prevPath);
    	
    	if (rc.canMove(direction) && !prevPathSet.contains(currentLocation.add(direction))) {
    		...
    		for (int i = 0; i < 8; i++) {
    			...
    			if (rc.canMove(testedDirection) && !prevPathSet.contains(currentLocation.add(testedDirection)) && targetLocation.distanceSquaredTo(currentLocation.add(testedDirection)) < closestDistance) {
    	...
    	prevPath.addLast(currentLocation);
    }
