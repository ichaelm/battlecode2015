package minerbot_maxoremindist_picky_savedmarkedminer4;
...
public class RobotPlayer {
	...
	*/
	...
	private static void mine() throws GameActionException {
		...
		if (myOre > minOre && !miningWouldBlock()) {
			rc.mine();
			markMining(rc.getID());
		} else {
			...
			for (int i = 0; i < 8; i++) {
				...
				if (rc.canMove(d) && dirOre > 0) {
					if (dirOre > bestOre) {
						...
						foundUnblocked = dirUnblocked;
					} else if (dirOre >= bestOre) {
						if (dirUnblocked && !foundUnblocked) {
							bestDirs[0] = d;
							numBestDirs = 1;
							bestOre = dirOre;
							bestHQDistSq = dirHQDistSq;
							foundUnblocked = dirUnblocked;
						} else if (dirUnblocked == foundUnblocked) {
	...
	private static boolean miningWouldBlock() {
		RobotInfo[] adjacent = rc.senseNearbyRobots(1, myTeam);
	...
	private static boolean miningWouldBlock(Direction dir) {
		RobotInfo[] adjacent = rc.senseNearbyRobots(rc.getLocation().add(dir), 1, myTeam);
