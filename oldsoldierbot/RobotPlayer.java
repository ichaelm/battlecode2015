...
public class RobotPlayer {
	...
	private static final int SOLDIER_ORDER_CHAN = 65516;
	private static final int SOLDIER_ORDER_NUM_COORDS_CHAN = 65517;
	private static final int SOLDIER_ORDER_MAX_COORDS = 7;
	private static final int[][] SOLDIER_ORDER_COORDS_CHANS = {{65518,65519},{65520,65521},{65522,65523},{65524,65525},{65526,65527},{65528,65529},{65530,65531}};
	...
	private static final int SOLDIER_ORDER_NONE = 0;
	private static final int SOLDIER_ORDER_DEFEND = 1;
	private static final int SOLDIER_ORDER_ATTACK_TOWERS = 2;
	private static final int SOLDIER_ORDER_ATTACK_HQ = 3;
	private static final int SOLDIER_ORDER_RALLY = 4;
	...
	public static void run(RobotController myrc) {
		...
		mapCenter = new MapLocation(HQLoc.x + (enemyHQLoc.x - HQLoc.x)/2, HQLoc.y + (enemyHQLoc.y - HQLoc.y)/2);
	...
	private static void runHQ() {
		...
		boolean soldiersRecooperating = false;
		...
		while (true) {
			try {
				...
				if (brandNew) {
					...
					rc.broadcast(SOLDIER_ORDER_CHAN, SOLDIER_ORDER_DEFEND);
					rc.broadcast(SOLDIER_ORDER_NUM_COORDS_CHAN, 1);
					rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][0], enemyHQLoc.x);
					rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][1], enemyHQLoc.y);
				...
				if (estimatedOreConsumption >= 1.5*estimatedOreGeneration || teamOre < 100) {
				...
				if (totalSupplyUpkeep*.8 >= totalSupplyGeneration) {
				...
				//this the the build order
				//HQBuildSoldier(progressRobotsByType, plannedTeamOre, 150); 
							...
							if (recieveBuildOrders(ID) == null) {
								if (source == RobotType.BEAVER) {
									...
									for (int k = 0; k < relativeBuildLocs.length; k++) {
										...
										if (!matchFound && rc.senseTerrainTile(buildLoc).isTraversable()) {
											success = true;
				...
				if (numEnemiesSwarmingBase() >= 10 || enemyTowerLocs.length < rc.senseTowerLocations().length) {
					...
					rc.broadcast(SOLDIER_ORDER_CHAN, SOLDIER_ORDER_DEFEND);
//					rc.broadcast(SOLDIER_ORDER_NUM_COORDS_CHAN, 1);
//					MapLocation moveLoc = locationWithLeastAllies(rc.getLocation(), myTowerLocs);
//					rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][0], moveLoc.x);
//					rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][1], moveLoc.y);
					// check if good time to stop swarming myself
				} else if (selfSwarmTimer <= 0 && numEnemiesSwarmingBase() < 3) {
					// check if good time to attack
					rc.setIndicatorString(0,"1");
					if (numSoldiers >= 50) { // && !soldiersRecooperating
						rc.setIndicatorString(0,"2");
						rc.broadcast(SOLDIER_ORDER_CHAN, SOLDIER_ORDER_ATTACK_TOWERS);
						rc.broadcast(SOLDIER_ORDER_NUM_COORDS_CHAN, enemyTowerLocs.length + 1);
						rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][0], enemyHQLoc.x);
						rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][1], enemyHQLoc.y);
						for (int i = 0; i < enemyTowerLocs.length; i++) {
							rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[i+1][0], enemyTowerLocs[i].x);
							rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[i+1][1], enemyTowerLocs[i].y);
						}
					} else { //check if a good time to retreat
						rc.setIndicatorString(0,"3 numSoldiers: " + numSoldiers);
						if (numSoldiers <= 30) {
							rc.setIndicatorString(0,"4");
//							soldiersRecooperating = true;
							rc.broadcast(SOLDIER_ORDER_CHAN, SOLDIER_ORDER_RALLY);
//							rc.broadcast(SOLDIER_ORDER_NUM_COORDS_CHAN, 1);
//							rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][0], (HQLoc.x + enemyHQLoc.x) / 2);
//							rc.broadcast(SOLDIER_ORDER_COORDS_CHANS[0][1], (HQLoc.y + enemyHQLoc.y) / 2);
						}
					}
				}
	...
	private static void HQBuild(int[] numFreeRobotsByType, int[] numRobotsByType, int[] progressRobotsByType, int plannedTeamOre, RobotType type, int desiredNumber) {
		int numBarracks = numRobotsByType[robotTypeToNum(RobotType.BARRACKS)] + progressRobotsByType[robotTypeToNum(RobotType.BARRACKS)];
		int numHelipads = numRobotsByType[robotTypeToNum(RobotType.HELIPAD)] + progressRobotsByType[robotTypeToNum(RobotType.HELIPAD)];
		int numAerospaceLabs = numRobotsByType[robotTypeToNum(RobotType.AEROSPACELAB)] + progressRobotsByType[robotTypeToNum(RobotType.AEROSPACELAB)];
		int numDrones = numRobotsByType[robotTypeToNum(RobotType.DRONE)] + progressRobotsByType[robotTypeToNum(RobotType.DRONE)];
		int numLaunchers = numRobotsByType[robotTypeToNum(RobotType.LAUNCHER)] + progressRobotsByType[robotTypeToNum(RobotType.LAUNCHER)];
		int numSoldiers = numRobotsByType[robotTypeToNum(RobotType.SOLDIER)] + progressRobotsByType[robotTypeToNum(RobotType.SOLDIER)];
		int numTechInstitutes = numRobotsByType[robotTypeToNum(RobotType.TECHNOLOGYINSTITUTE)] + progressRobotsByType[robotTypeToNum(RobotType.TECHNOLOGYINSTITUTE)];
		int numMinerFactories = numRobotsByType[robotTypeToNum(RobotType.MINERFACTORY)] + progressRobotsByType[robotTypeToNum(RobotType.MINERFACTORY)];
		int numComputers = numRobotsByType[robotTypeToNum(RobotType.COMPUTER)] + progressRobotsByType[robotTypeToNum(RobotType.COMPUTER)];

		switch (type) {
		case SUPPLYDEPOT: // goal: build a supply depot
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType, plannedTeamOre, RobotType.BEAVER, 1);
			} else {
				// goal: build a supply depot
				order(RobotType.BEAVER, RobotType.SUPPLYDEPOT);
				plannedTeamOre -= RobotType.SUPPLYDEPOT.oreCost;
			}
			break;
		case MINERFACTORY: // goal: build a miner factory
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType, plannedTeamOre, RobotType.BEAVER, 1);
			} else {
			...
			break;
		case BARRACKS: // goal: build a barracks
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType, plannedTeamOre, RobotType.BEAVER, 1);
			} else {
			...
			break;
		case TANKFACTORY: // goal: build a tank facotry
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType, plannedTeamOre, RobotType.BEAVER, 1);
			} else {
				if (numBarracks < 1) {
				// goal: build a barracks
					HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType, plannedTeamOre, RobotType.BARRACKS, 1);
				} else {
					// goal: build an tank factory
					order(RobotType.BEAVER, RobotType.TANKFACTORY);
					plannedTeamOre -= RobotType.TANKFACTORY.oreCost;
				}
			}
			break;
		case HELIPAD: // goal: build a barracks
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.BEAVER, 1);
			} else {
			...
			break;
		case AEROSPACELAB: // goal: build an aerospace lab
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.BEAVER, 1);
			} else {
				if (numHelipads < 1) {
				// goal: build a helipad
					HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.HELIPAD, 1);
				} else {
			...
			break;
		case TECHNOLOGYINSTITUTE: // goal: build a technology institute
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.BEAVER, 1);
			} else {
				// goal: build a technology institute
				order(RobotType.BEAVER, RobotType.TECHNOLOGYINSTITUTE);
				plannedTeamOre -= RobotType.TECHNOLOGYINSTITUTE.oreCost;
			}
			break;
		case TRAININGFIELD: // goal: build a training field
			if (numFreeRobotsByType[robotTypeToNum(RobotType.BEAVER)] < 1) {
				// goal: build a beaver
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.BEAVER, 1);
			} else {
				if (numTechInstitutes < 1) {
				// goal: build a tech institute
					HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.TECHNOLOGYINSTITUTE, 1);
				} else {
					// goal: build a training field
					order(RobotType.BEAVER, RobotType.TRAININGFIELD);
					plannedTeamOre -= RobotType.TRAININGFIELD.oreCost;
				}
			}
			break;
		case BEAVER: // goal: build a beaver
			order(RobotType.HQ, RobotType.BEAVER);
			plannedTeamOre -= RobotType.BEAVER.oreCost;
			break;
		case MINER:
			// goal: build more miners
			if (numMinerFactories < 1 || numFreeRobotsByType[robotTypeToNum(RobotType.MINERFACTORY)] < 1) {
				// goal: build a miner factory
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.MINERFACTORY, 1);
			} else {
				// goal: build a miner
				order(RobotType.MINERFACTORY, RobotType.MINER);
				plannedTeamOre -= RobotType.MINER.oreCost;
			...
			break;
		case COMPUTER:
			// goal: build more computers
			if (numTechInstitutes < 1 || numFreeRobotsByType[robotTypeToNum(RobotType.TECHNOLOGYINSTITUTE)] < 1) {
				// goal: build a tech institute
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.TECHNOLOGYINSTITUTE, 1);
			} else {
				// goal: build a computer
				order(RobotType.TECHNOLOGYINSTITUTE, RobotType.COMPUTER);
				plannedTeamOre -= RobotType.COMPUTER.oreCost;
			}
			break;
		case SOLDIER:
			// goal: build more soldiers
			if (numBarracks < 1 || numFreeRobotsByType[robotTypeToNum(RobotType.BARRACKS)] < 1) {
				// goal: build a barracks
				HQBuild(numFreeRobotsByType, numRobotsByType, progressRobotsByType,plannedTeamOre, RobotType.BARRACKS, 1);
			} else {
								...
								if (rc.getTeamOre() < buildOrder.oreCost) {
									...
									if (orderLoc.x == NO_BOUND) {
										...
										if (success) {
											...
											tryMove(myLoc.directionTo(mapCenter));
										}
...
//					attackSomething();
				...
				if (rc.isCoreReady()) {
					int order = rc.readBroadcast(SOLDIER_ORDER_CHAN);
					if (order == SOLDIER_ORDER_ATTACK_TOWERS) {
						int numCoords = rc.readBroadcast(SOLDIER_ORDER_NUM_COORDS_CHAN);
						MapLocation[] targets = new MapLocation[numCoords];
						for (int i = 0; i < numCoords; i++) {
							targets[i] = new MapLocation(rc.readBroadcast(SOLDIER_ORDER_COORDS_CHANS[i][0]), rc.readBroadcast(SOLDIER_ORDER_COORDS_CHANS[i][1]));
						}
						tryMove(rc.getLocation().directionTo(closestLocation(mapCenter, targets)));
					} else  if (order == SOLDIER_ORDER_DEFEND) {
						harasserMoveTo(closestLocation(mapCenter, myTowerLocs));
					} else if (order == SOLDIER_ORDER_RALLY) {
						rally();
					}
				}
				transferSupply();
	...
	//determines if building a structure would congest traffic flow
	private static boolean isCongesting(int radius) {
	...
	private static void soldierSwarmUnits(MapLocation[] targets) throws GameActionException {
		MapLocation myLoc = rc.getLocation();
	...
	private static boolean isInEnemyTowerRange(MapLocation loc) {
		for (MapLocation i: enemyTowerLocs) {
			if (loc.distanceSquaredTo(i) <= 30) {
				return true;
			}
		}
		if (inEnemyHQRange(loc)) {
			return true;
		}
		return false;
	}
	...
	private static void rally() throws GameActionException {
		...
		if (enemy != null) {
			...
			if (invaderLoc != null) {
				...
				harasserMoveTo(mapCenter);
//				if (brandNew) {
//					// initial location finding
//					minerTarget = findNearestMarkedMiner();
//					if (minerTarget == null) {
//						tryMove(rc.getLocation().directionTo(closestTower(mapCenter, myTowerLocs)));
//					} else {
//						tryMove(myLoc.directionTo(minerTarget));
//					}
//				} else {
...
//				}
	...
	private static MapLocation locationWithLeastAllies(MapLocation center, MapLocation[] locations) {
		MapLocation destinationLocation = null;
		int numAllies = 1000;
		int testAllies;
		for (MapLocation i: locations) {
			testAllies = rc.senseNearbyRobots(24, myTeam).length;
			if (testAllies < numAllies) {
				destinationLocation = i;
				numAllies = testAllies;
			}
		}
	...
	private static boolean tryBuild(Direction d, RobotType type) throws GameActionException {
		...
		int[] offsets = {0,1,-1,2,-2,3,-3,4};
		int dirint = directionToInt(d);
		while (offsetIndex < 8 && !rc.canBuild(directions[(dirint+offsets[offsetIndex]+8)%8], type)) {
			offsetIndex++;
		}
		if (offsetIndex < 8) {
