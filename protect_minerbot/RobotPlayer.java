...
public class RobotPlayer {
	...
	private static final int DEFEND_MINER_CHAN = UNIT_NEEDS_SUPPLY_Y_CHAN + 1;
		...
		while (true) {
			try {
				...
				if (rc.isCoreReady()) {
					if (Clock.getRoundNum() > 1700) {
						if (enemyTowerLocs.length == 0) {
							moved = tryMoveReturns(myLoc.directionTo(enemyHQLoc));
						} else {
							moved = tryMoveReturns(myLoc.directionTo(closestLocation(mapCenter, enemyTowerLocs)));
						}
					} else {
						Direction escapeDir = escapeCrowding();
						if (escapeDir != null) {
							moved = launcherTryMoveReturns(escapeDir);
						} else {
							mine();
						}
					}
	...
	private static void runSoldier() {
		int buddyMinerChan = 0;
		boolean buddyIsAlive = true;
		try {
			//if(numRobotsByType[SOLDIER.ordinal()] < numRobotsByType[MINER.ordinal()]){
				boolean buddyFound = false;
				int whichMiner = 1;
				while(!buddyFound){
					if(rc.readBroadcast(DEFEND_MINER_CHAN + 3 * whichMiner) == 0){
						rc.broadcast(DEFEND_MINER_CHAN + 3 * whichMiner, rc.getID());
						buddyFound = true;
						buddyMinerChan = DEFEND_MINER_CHAN + 3 * whichMiner - 2;
					} else
						whichMiner++;
				}
			//}
		...
		while (true) {
			try {
				...
				//find buddy
				MapLocation buddyMinerLoc = null;
				if(buddyMinerChan != 0){
				...
				if (rc.isCoreReady()) {
					if(buddyMinerLoc == null || !buddyIsAlive){
						...
						if(buddy == null || buddy.type != RobotType.MINER){
							...
							System.out.println(rc.getID());
							if (Clock.getRoundNum() < 1500) {
								tryMove(myLoc.directionTo(buddyMinerLoc));
							} else {
								if (enemyTowerLocs.length == 0) {
									tryMove(myLoc.directionTo(enemyHQLoc));
								} else {
	...
	private static Direction tryMoveReturns(Direction d) throws GameActionException {
		int offsetIndex = 0;
		int[] offsets = {0,1,-1,2,-2};
		int dirint = directionToInt(d);
		while (offsetIndex < 5 && !rc.canMove(directions[(dirint+offsets[offsetIndex]+8)%8])) {
	...
	private static Direction launcherTryMoveReturns(Direction d) throws GameActionException {
		int offsetIndex = 0;
		int[] offsets = {0,1,-1,2,-2};
		int dirint = directionToInt(d);
		while (offsetIndex < 5 && (!rc.canMove(directions[(dirint+offsets[offsetIndex]+8)%8]) || inEnemyBuildingRange(rc.getLocation().add(directions[(dirint+offsets[offsetIndex]+8)%8])))) {
