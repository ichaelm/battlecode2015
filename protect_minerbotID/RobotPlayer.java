...
//						launcherRally();
	...
	private static void runMiner() {
		try {
				...
				int whichMiner = 0;
				while(!buddyFound){
					if(rc.readBroadcast(DEFEND_MINER_CHAN + 2 * whichMiner) == 0 || rc.readBroadcast(DEFEND_MINER_CHAN + 2 * whichMiner) == 1){
						rc.broadcast(DEFEND_MINER_CHAN + 2 * whichMiner, rc.getID());
						buddyFound = true;
					} else
						whichMiner++;
				}
	...
	private static void runSoldier() {
		...
		try {
				...
				int whichMiner = 0;
				while(!buddyFound){
					if(rc.readBroadcast(DEFEND_MINER_CHAN + 2 * whichMiner + 1) == 0 || rc.readBroadcast(DEFEND_MINER_CHAN + 2 * whichMiner + 1) == 1){
						rc.broadcast(DEFEND_MINER_CHAN + 2 * whichMiner + 1, rc.getID());
						buddyFound = true;
						buddyMinerChan = DEFEND_MINER_CHAN + 2 * whichMiner;
					} else
						whichMiner++;
				}
		} catch (Exception e) {
		...
		while (true) {
			try {
				...
				//find buddy
				int buddyID = rc.readBroadcast(buddyMinerChan);
				
				// move according to orders
				if (rc.isCoreReady()) {
					if(buddyMinerChan == 0 || !rc.canSenseRobot(buddyID)){
						...
						if (Clock.getRoundNum() < 1500) {
							RobotInfo buddy = rc.senseRobot(buddyID);
							tryMove(myLoc.directionTo(buddy.location));
						} else {
							if (enemyTowerLocs.length == 0) {
								tryMove(myLoc.directionTo(enemyHQLoc));
							} else {
