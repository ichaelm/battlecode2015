package minerbot_maxoremindist_picky_savedmarkedminer_left;
			...
			if (bestOre > minOre) {
				...
				if (brandNew) {
					...
					if (minerTarget == null) {
						...
						tryMoveRight(myLoc.directionTo(minerTarget));
					}
					unmarkMining(rc.getID());
				} else {
					if (minerTarget != null) {
						...
						if (myLoc.isAdjacentTo(minerTarget) || myLoc.equals(minerTarget)) {
							...
							minerTarget = null;
							tryMoveRight(myLoc.directionTo(HQLoc).opposite());
							unmarkMining(rc.getID());
						} else {
							// not reached target, continue initial location finding
							boolean success = tryMoveRight(myLoc.directionTo(minerTarget));
							if (!success) {
								minerTarget = null;
								tryMoveRight(myLoc.directionTo(HQLoc).opposite());
							}
